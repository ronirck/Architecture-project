...
using namespace std;    // Para usar cout, endl, etc.
using namespace chrono; // Para usar milliseconds
...
//     auto start = high_resolution_clock::now(); // Inicia la medición del tiempo
...
//     auto end = high_resolution_clock::now(); // Finaliza la medición del tiempo
...
int selectPatient(const vector<Patient> &patients)
{
    ...
    size_t endIdx = min(static_cast<size_t>(startIdx + patientsPerPage - 1), patients.size() - 1);
